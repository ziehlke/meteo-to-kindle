...
class Airly:
    ...
    def plot_caqi_history(self):
        ...
        plt.savefig('caqi.png', bbox_inches='tight')

    def fill_template(self):
        # load fonts
        open_moji = ImageFont.truetype('Manrope/OpenMoji-Black.ttf', 60)
        manrope_extra_bold = ImageFont.truetype('Manrope/Manrope-ExtraBold.ttf', 40)
        manrope_bold_small = ImageFont.truetype('Manrope/Manrope-Bold.ttf', 30)
        manrope_regular_small = ImageFont.truetype('Manrope/Manrope-Regular.ttf', 15)
        current_caqi = self.data['current']['indexes'][0]['value']
        emoji_index = np.digitize(current_caqi, self.CAQI_BINS)

        image = Image.open('master_template.png')
        draw = ImageDraw.Draw(image)
        draw.text((9, 653), self.EMOJIS[emoji_index], fill="black", font=open_moji)
        advice = self.data['current']['indexes'][0]['advice']
        draw.text((9, 740), '\n'.join(textwrap.wrap(advice, width=50)), fill="black", font=manrope_regular_small)
        draw.text((70, 650), str(round(current_caqi)), fill="black", font=manrope_extra_bold)
        draw.text((195, 650), str(round(self.data['current']['values'][2]['value'])), fill="black",
                  font=manrope_extra_bold)
        draw.text((350, 650), str(round(self.data['current']['values'][1]['value'])), fill="black",
                  font=manrope_extra_bold)
        draw.text((505, 650), str(round(self.data['current']['values'][0]['value'])), fill="black",
                  font=manrope_extra_bold)
        ...
        draw.text((350, 720), str(round(self.data['current']['values'][5]['value'])), fill="black",
                  font=manrope_extra_bold)
        draw.text((505, 720), str(round(self.data['current']['values'][4]['value'])), fill="black",
                  font=manrope_extra_bold)
